...
namespace P6
{
    public partial class mainForm : Form
    {
        ...
        private void updateGUI()
        {
            for (int r = 0; r <= grid.GetUpperBound(0); r++)
                for (int c = 0; c <= grid.GetUpperBound(1); c++)
                {
                    ...
                    {
                        grid[r, c].BackgroundImage = P6.Properties.Resources.four;
                        grid[r, c].BackgroundImageLayout = ImageLayout.Stretch;
                    }
                    else if (a == "5")
                    {
                        grid[r, c].BackgroundImage = P6.Properties.Resources.five;
                        grid[r, c].BackgroundImageLayout = ImageLayout.Stretch;
                    }
                    else if (a == "-")
                    {
        ...
        private void statusButton_Click(object sender, EventArgs e)
        {
            mainTimer.Stop();
            timerReset();
            myGame = new Minesweeper(currentSize, currentSize);
            updateGUI();
            checkStatus();
            mainTimer.Start();
        }
